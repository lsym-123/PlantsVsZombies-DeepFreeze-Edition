...
LawnApp::LawnApp()
{
	...
	std::string aTitleName = "植物大战僵尸极寒版 Plants vs. Zombies DeepFreeze Edition v1.5";
#ifdef _DEBUG
	aTitleName += " BETA ";
...
{
	LawnDialog* aComfirmDialog = (LawnDialog*)DoDialog(Dialogs::DIALOG_STORE_PURCHASE, true, _S("       "), theMessage, _S(""), Dialog::BUTTONS_YES_NO);
...
{
	if (theId % 10000 >= 2000 && theId % 10000 < 3000)  //      theId   [2000, 3000)        theId - 2000             
	{
		switch (theId - 2000)
		{
	...
	if (theId % 10000 >= 3000 && theId < 4000)  //      theId   [3000, 4000)        theId - 3000             
	{
		switch (theId - 3000)
		{
		case Dialogs::DIALOG_PREGAME_NAG:
...
SeedType LawnApp::GetAwardSeedForLevel(int theLevel)
{
	...
	int aSeedsHasGot = (aArea - 1) * 8 + aSub;  //              8             1    
	if (aSub >= 10)
	{
		aSeedsHasGot -= 2;  //     10           2           
	}
